...
function preload(){
  ...
  speeches[1] = loadSound("./soundfiles/candy.mp3");
  speeches[2] = loadSound("./soundfiles/speech_es_short.mp3");
...
function makeSound(osc_index, pta, ptb) {
...
if(startPlaying){
  ...
  if (speeches[osc_index].isPlaying()) {
    speeches[osc_index].setVolume(
      map(dist(pta.x, pta.y, ptb.x, ptb.y), 0, dia_length, 0.1, 0.8)
    );
  } else {
...
function draw_triangulation(triangulation) {
  ...
  for (i = 0; i < triangulation.length / 3; ++i) {
    ...
    makeSound(i, p[triangulation[i + 0]], p[triangulation[i + 1]]);
